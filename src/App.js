  ...
  try {
    const response = await axios.post(
      'https://api.openai.com/v1/chat/completions',
      {
        model: 'gpt-3.5-turbo',
        messages: [
          { role: "user", content: `Provide feedback on english grammer error and better word sugestion for following. ${inputText}` },
        ],
        max_tokens: 700,
        temperature: 0
      },
    ...
    const message = 'GPT is too busy right now. Try again later';
    return message;
    // throw new Error('Failed to send message to ChatGPT');
  }
}
...
const App = () => {
  ...
  const apiKey = process.env.REACT_APP_GOOGLE_API_KEY;
    // Function to get the microphone frequency
    const getMicFrequency = () => {
      // Logic to get microphone frequency
      // For demonstration, I'm randomly generating a frequency
      const frequency = Math.floor(Math.random() * 10000) + 1000;
      setMicFrequency(frequency);
    };
  
    // Call getMicFrequency function every second
    useEffect(() => {
      ...
      recorder.addEventListener('dataavailable', async (event) => {
        ...
        try {
          ...
          if (response.data.results && response.data.results.length > 0) {
            ...
            if(output && output !== 'GPT is too busy right now. Try again later') {
...
const default_mode = (
  <div style={{ background: '#F1F3F5', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', fontFamily: 'Roboto, sans-serif' }}>
    ...
    {!recording ? (
      <button onClick={startRecording} style={{ background: '#4A90E2', color: 'white', fontSize: '24px', padding: '10px 20px', borderRadius: '5px', border: 'none', cursor: 'pointer', marginBottom: '20px', boxShadow: '0 3px 5px rgba(0,0,0,0.3)' }}>Start English Speaking</button>
    ) : (
      <button onClick={stopRecording} style={{ background: '#F87676', color: 'white', fontSize: '24px', padding: '10px 20px', borderRadius: '5px', border: 'none', cursor: 'pointer', marginBottom: '20px', boxShadow: '0 3px 5px rgba(0,0,0,0.3)' }}>Stop and Get Feedback</button>
    )}
...
const matrix_mode = (
  <div style={{ background: '#000000', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', fontFamily: 'Courier New, monospace' }}>
    ...
    {!recording ? (
      <button onClick={startRecording} style={{ background: '#000000', color: '#00FF00', fontSize: '24px', padding: '10px 20px', borderRadius: '5px', border: '2px solid #00FF00', cursor: 'pointer', marginBottom: '20px', boxShadow: '0 3px 5px rgba(0,255,0,0.3)' }}>Start English Speaking</button>
    ) : (
      <button onClick={stopRecording} style={{ background: '#000000', color: '#00FF00', fontSize: '24px', padding: '10px 20px', borderRadius: '5px', border: '2px solid #00FF00', cursor: 'pointer', marginBottom: '20px', boxShadow: '0 3px 5px rgba(0,255,0,0.3)' }}>Stop and Get Feedback</button>
    )}
